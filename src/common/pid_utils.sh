...
function pid_guard() {
  ...
  echo "------------ pid_guard: STARTING $(basename "$0") at $(date) --------------" | tee /dev/stderr
  ...
  if pid_is_running "${pid}"; then
    echo "pid_guard: ${name} is already running (pid=${pid} / ${pidfile}), please stop it first"
    exit 1
  fi

  echo "pid_guard: removing stale pidfile: ${pidfile}"
  rm "${pidfile}"
}
